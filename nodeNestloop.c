...
ExecNestLoop(PlanState *pstate)
{
	...
	{
		/*
		 * If we have an empty or out-of-time outer block, get a new block and reset the
		 * inner scan.
		 */
        ...
        for (int i = 0; i < node->Bsize; i++)
		{
			...
			/*
			 * Ftch the values of any outer Vars and store them in the appropriate PARAM_EXEC slots.
			 */
			...
			foreach(lc, nl->nestParams)
			{
				...
				prm->value = slot_getattr(node->outerBlock[i],
